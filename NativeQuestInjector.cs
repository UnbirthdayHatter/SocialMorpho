...
public unsafe class NativeQuestInjector : IDisposable
{
    ...
    private void InjectCustomQuests()
    {
        ...
        for (int i = 0; i < activeQuests.Count && customQuestStartIndex + i < 10; i++)
        {
            ...
            // Store original pointers ONLY if we haven't already stored them
    ...
    private void FreeAllocatedStrings()
    {
        foreach (var ptr in AllocatedStrings)
        {
            ...
            {
                Plugin.PluginLog.Warning($"Failed to free allocated string: {ex.Message}");
            }
        }
    ...
    private void RestoreOriginalState()
    {
        try
        {
            ...
            {
                // Only restore if the pointer is still valid (not null)
    ...
    public void Dispose()
    {
        ...
        {
            Plugin.PluginLog.Error($"Error during disposal: {ex}");
        }
    }
}